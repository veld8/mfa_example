defmodule MfaExampleWeb.UserSettingsLiveTest do
  ...
  describe "Index (2FA)" do
    ...
    test "enables totp secret", %{conn: conn} do
      {:ok, settings_live, _html} = live(conn, Routes.user_settings_path(conn, :edit))
    ...
    test "disables 2FA password", %{conn: conn, user: user} do
      ...
      refute MfaExample.Repo.get(MfaExample.Accounts.UserTOTP, totp.id)
    end
    ...
    test "changes totp secret", %{conn: conn, user: user} do
      ...
      assert MfaExample.Repo.get!(MfaExample.Accounts.UserTOTP, totp.id).secret !=
               valid_totp_secret()
    end
    ...
    test "regenerates backup codes", %{conn: conn, user: user} do
      ...
      updated_totp = MfaExample.Repo.get!(MfaExample.Accounts.UserTOTP, totp.id)
